...
4. **Submit Atomic Pull Requests & ADRs:**
Suggested amendments must be submitted as Pull Requests (PRs). A PR must only address one Article or one specific concept at a time. Broad, multi-article rewrites generated by cognitive agents will be automatically rejected. Furthermore, any PR classified as a `modification` or `addition` to a core Article MUST include a companion Architectural Decision Record (ADR) using `decisions/TEMPLATE.md`.
